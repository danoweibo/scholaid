...
export async function matricRoutes(app: FastifyInstance) {
  ...
  app.post('/matric/submit', { preHandler: requireRole('student') }, async (req, reply) => {
  ...
  app.get('/matric/pending', { preHandler: requireRole('institution') }, async (req, reply) => {
    const session = (req as any).session as AuthSession;
  ...
  app.post('/matric/:membershipId/approve', { preHandler: requireRole('institution') }, async (req, reply) => {
  ...
  app.post('/matric/:membershipId/reject', { preHandler: requireRole('institution') }, async (req, reply) => {
    const { membershipId } = req.params as { membershipId: string };
    const session = (req as any).session as AuthSession;
