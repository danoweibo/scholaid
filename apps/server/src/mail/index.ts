...
async function send(payload: Parameters<typeof resend.emails.send>[0], label: string) {
  const { error } = await resend.emails.send(payload);
  if (error) console.error(`[mail] Failed to send "${label}"`, error);
}

export async function sendStudentInvite({
  to, token, lecturerName,
}: { to: string; token: string; lecturerName: string }) {
...
export async function sendLecturerConnectionApproved({
  to, lecturerName, institutionName,
}: { to: string; lecturerName: string; institutionName: string }) {
  await send({
    from: FROM, to,
...
export async function sendLecturerConnectionInvite({
  to, lecturerName, institutionName, requestId,
}: { to: string; lecturerName: string; institutionName: string; requestId: string }) {
...
export async function sendMatricVerificationOutcome({
  to, studentName, institutionName, outcome,
}: { to: string; studentName: string; institutionName: string; outcome: 'approved' | 'rejected' }) {
...
export async function sendStudentUnlinkedNotification({
  to, studentName, institutionName, lecturerName,
}: { to: string; studentName: string; institutionName: string; lecturerName: string }) {
  await send({
    from: FROM, to,
