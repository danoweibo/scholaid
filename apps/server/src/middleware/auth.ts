 ...
 */
export async function getSession(req: FastifyRequest): Promise<AuthSession | null> {
 ...
 */
export async function requireAuth(req: FastifyRequest & { session?: AuthSession }, reply: FastifyReply) {
  const session = await getSession(req);
  if (!session) {
    return reply.status(401).send({ statusCode: 401, message: 'Unauthorized', error: 'Unauthorized' });
  }
  req.session = session;
}
...
export function requireRole(...roles: ScholaidUser['scholaidRole'][]) {
  return async function (req: FastifyRequest & { session?: AuthSession }, reply: FastifyReply) {
 ...
 */
export async function optionalAuth(req: FastifyRequest & { session?: AuthSession | null }) {
