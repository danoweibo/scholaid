...
export async function submit(
  ...
  if (!student)
    throw { statusCode: 403, message: "Student profile not found." };
  ...
  if (!membership)
    throw {
      statusCode: 404,
      message: "No pending membership found for this institution.",
    };
  if (membership.status === "full")
    throw {
      statusCode: 400,
      message: "You are already a full member of this institution.",
    };
  if (membership.matricVerificationStatus === "approved")
    throw { statusCode: 400, message: "Matric number already verified." };
  ...
  if (institution.matricVerificationMode === "programmatic") {
    const verified = await runProgrammaticCheck(
      session.user.name,
      body.matricNumber,
      institution.id,
    );
...
export async function approve(membershipId: string, session: AuthSession) {
  ...
  if (membership.matricVerificationStatus === "approved")
    throw { statusCode: 400, message: "Already approved." };
  await grantFullMembership(membership.id);
  await notifyStudent(
...
async function getMembershipForInstitution(
  membershipId: string,
  session: AuthSession,
) {
  const [institution] = await db
...
async function notifyStudent(
  ...
  await mail.sendMatricVerificationOutcome({
    to: userRow.email,
