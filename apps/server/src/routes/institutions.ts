...
import * as institutionsService from "@/services/institutions.js";
...
export async function institutionRoutes(app: FastifyInstance) {
  ...
  app.post(
    ...
    { preHandler: requireRole("lecturer") },
    async (req, reply) => {
      const session = (req as any).session as AuthSession;
      const result = await institutionsService.lecturerConnect(
        session,
        req.body as any,
      );
  ...
  app.post(
    "/institutions/requests/:requestId/accept",
    { preHandler: requireRole("lecturer") },
    async (req, reply) => {
  ...
  app.post(
    "/institutions/requests/:requestId/approve",
    { preHandler: requireRole("institution") },
    async (req, reply) => {
      const { requestId } = req.params as { requestId: string };
      const session = (req as any).session as AuthSession;
      const result = await institutionsService.approveRequest(
        requestId,
        session,
      );
      return reply.send(result);
  ...
  app.post(
    "/institutions/lecturers/:lecturerId/remove",
    { preHandler: requireRole("institution") },
    async (req, reply) => {
