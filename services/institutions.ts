...
export async function lecturerConnect(
  ...
  if (lecturer.institutionConnection === "verified")
    throw {
      statusCode: 409,
      message: "Already connected to an institution. Leave first.",
    };

  const [institution] = await db
    .select()
    .from(institutions)
    .where(eq(institutions.institutionId, body.institutionId));
  if (!institution)
    throw {
      statusCode: 404,
      message: `Institution "${body.institutionId}" not found.`,
    };
  ...
  const [request] = await db
    ...
    .values({
      ...
      initiator: "lecturer",
      staffEmailDomain: body.staffEmailDomain,
...
export async function approveRequest(requestId: string, session: AuthSession) {
  ...
  if (!institution)
    throw { statusCode: 403, message: "Institution profile not found." };
  ...
  if (!request)
    throw { statusCode: 404, message: "Pending request not found." };
  ...
  if (lecturerUser)
    await mail.sendLecturerConnectionApproved({
...
export async function institutionInviteLecturer(
  session: AuthSession,
  ...
  const [lecturer] = await db
    .select()
    .from(lecturers)
    .where(eq(lecturers.lecturerId, body.lecturerId));
  if (!lecturer)
    throw {
      statusCode: 404,
      message: `Lecturer "${body.lecturerId}" not found.`,
    };
  if (lecturer.institutionConnection === "verified")
    throw {
      statusCode: 409,
      message: "This lecturer is already connected to an institution.",
    };
  ...
  if (existing)
    throw {
      statusCode: 409,
      message: "A pending request already exists for this lecturer.",
    };
  ...
  if (lecturerUser)
    await mail.sendLecturerConnectionInvite({
      to: lecturerUser.email,
      lecturerName: lecturerUser.name,
      institutionName: institution.name,
      requestId: request.id,
...
export async function lecturerAcceptInvite(
  requestId: string,
  session: AuthSession,
) {
  const [lecturer] = await db
  ...
  if (lecturer.institutionConnection === "verified")
    throw {
      statusCode: 409,
      message: "You are already connected to an institution.",
    };
  ...
  if (!request)
    throw { statusCode: 404, message: "Pending institution invite not found." };
...
export async function removeLecturer(
  ...
  if (!lecturer)
    throw {
      statusCode: 404,
      message: "Verified lecturer not found under this institution.",
    };
...
export async function lecturerLeave(session: AuthSession) {
  ...
  if (lecturer.institutionConnection !== "verified")
    throw {
      statusCode: 400,
      message: "You are not currently connected to an institution.",
    };
...
async function unlinkExclusiveStudents(
  lecturerId: string,
  ...
  const exclusiveIds = studentIds.filter(
    (id: string) => !otherCoveredIds.includes(id),
  );
  ...
  for (const studentId of exclusiveIds) {
    ...
    if (studentUser && institutionRow && lecturerRow)
      await mail.sendStudentUnlinkedNotification({
